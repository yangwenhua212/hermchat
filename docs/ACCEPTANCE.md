...
- [ ] 助手气泡喇叭可朗读；设置「自动朗读」时边出字边读，且只读本轮刚生成一次（离开再进不重读）；系统朗读：设置选「系统」后气泡喇叭应有声（系统测听有声时）
...
| 2026-08-05 | | 0.1.30+ | 真机：自动朗读离开再进又读一遍 | 进页 prime + Application 级 autoHandled；规则 `.cursor/rules/auto-speak.mdc` |
