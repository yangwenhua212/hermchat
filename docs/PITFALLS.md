...
| HxMV 页显示「连不上 HxMV」，但服务端 `/api/health` 明明 200 | 手机到 CDN 的连接被**中途重置或直接悬挂**（实测：手机第一次请求根本没到服务器，隔一会儿那次 200；再点「保存并检测」界面**毫无反应**——其实是在等超时）；旧版只在打开页面时探一次、失败无重试入口，映射表也没覆盖「HTTP/2 流重置 / 连接被关闭 / 响应状态行异常」→ 糊成兜底文案 | 探测**换短超时的 probe 客户端**（连 8s/读 12s，别用「连 15s+读 60s」把用户晾 75 秒）；失败**自动再来一次**；状态行先显示「正在连接 …」再出结果 + 常驻**「重试」**按钮；兜底文案补真实原因（`连不上 HxMV · stream was reset`）；映射表补流重置/连接关闭/gateway 类关键词。**排查手法**：`sudo grep okhttp /var/log/nginx/access.log` —— 有 OK 请求 = 服务端没问题，是手机那条路；**日志里没有新请求也有意义**：说明请求压根没发出去/卡在连上 |
