...
| 7 | 快捷指令栏 + 输入偏好 | ✅ 点选插入/发送；排序可持久化；语音/文字/混合 —— **0.1.38 起提示词栏下线**（设置入口同步删除） |
...
聊天定制说明：齿轮进入设置（主题色含背景 / 气泡 / 壁纸相册与搜索 / 输入 / 网关 / 搜索）；Agent 管理在顶栏下拉；历史列表回看旧聊；「新建对话」换本地会话 + 服务端 Session。

配置进阶说明：`EndpointProbe` + 竖屏扫码 + 粘贴导入。

---

## 并行探索：HxSync for Robotics（具身 · 另开一条能力线）

> **定位**：机器人相关是 App 的**又一个功能域 / 档位向扩展**，**不替代、不架空**现有 ①②③④ 主线（聊天、连 Agent、端侧网关、本机工具）。  
> 主线继续服务「口袋里的 Agent 客户端」；具身线只是在同一套 Loop / 确认卡 / 多档调度上**多接一类执行器**。  
> 总调见仓库根目录 [VISION.md](../VISION.md)（两翼并进 / 开闭原则）。

一句话（愿景，非当前交付）：手机可作为机器人的自然语言界面、安全确认闸门与（远期）感知/决策节点之一——**并列于**现有聊天与本机工具，而不是「改造成只做机器人」。

### 与主线的关系

| | 主线（现在～v0.3） | 具身线（探索） |
|--|-------------------|----------------|
| 用户价值 | 连电脑/云 Agent、出门网关、本机闹钟日历等 | 控制实体机器人（底盘/简单臂） |
| 代码策略 | 继续打磨 ③④、工具、真机矩阵 | **另增**协议薄层 + `robot.*` 工具；复用 Loop/确认卡 |
| 是否替换四档 | — | **否**；最多将来多一种连接类型或工具命名空间 |

### 阶段（探索用，不绑死发版号）

| 阶段 | 目标 | 做什么 | 明确不做什么 |
|------|------|--------|----------------|
| **准备（已有）** | 复用基础设施 | ③ 长连接/流式、④ Loop 阶段、确认卡、识图入口、多档路由 —— 当作「人在回路」外壳 | 不宣称已是机器人 OS |
| **竖切原型（优先）** | 一台低成本 ROS 2 小车：「向前走」能动、屏上有状态 | WebSocket→ROS Bridge 或少数话题；`robot.move` / `stop` 等 **写操作必确认**；先遥控成功 | 不上全量 jros2、不上抓取导航全家桶、不改写四档产品 |
| **加厚（其后）** | 手机传感器作可选话题、更多原语 | 相机/IMU 等按需发布 | 不承诺毫秒级运动反射（反射在车端实时层） |
| **远期愿景** | 端侧 VLA / 多机协同 | 单独立项探索；① 仍是聊天小模型，**不自动变成** VLA | 不把 Gemini Robotics On-Device 当排期承诺 |

### 工程原则

1. **薄协议、厚复用**：优先 Bridge / 少量 `robot.*` 进现有 `ToolRegistry` + `ToolRisk`，而不是先大重构 `Action` sealed 层级。  
2. **安全同主线**：移动/抓取一律确认卡；取消即停。  
3. **主线带宽优先**：真机矩阵、③④ 稳定、PITFALLS 收口 > 具身探索。具身不抢主线发版节奏。  
4. 文档与坑记入 [PITFALLS.md](PITFALLS.md) / 本页；用户可见能力未落地前 **README 已具备表不写机器人当现成功能**。

验收竖切（将来勾）：对 ROS 2 小车说「向前走 1 米」→ 确认 → 车动 → 气泡/中间态可见阶段。
