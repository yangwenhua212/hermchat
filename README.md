...
| **④ Agent Loop** | 分析 → 确认 → 执行 → 观察 分阶段上屏；写操作须确认；坏 JSON 纠正；首包超时可改本地/备用；可选 ③ 失败持久降级 |
| **本机工具** | 闹钟/日历/开链/搜索/分享/剪贴板/开应用/拨号/地图/邮件/本机记忆（见 [REMOTE_BRAIN_LOCAL_TOOLS.md](docs/REMOTE_BRAIN_LOCAL_TOOLS.md)） |
...
以下**不是**当前预览版（0.1.28）能力，写在这里只为讲清方向。
...
已有发送失败后的本轮 `AgentFailover`（③↔④，可落到 ②）；**不永久改当前 Agent**。  
**可选持久降级**：设置根目录「连接失败自动降级」开关（默认关）。③ 连接/发送失败时切到 ④>②>①，顶栏可切回；与本轮备用独立。  
