...
| ④ | **端侧网关** | 本地 + API 混合路由；API 路径为轻量 **Agent loop**（中间态 / 分级确认 / 超步数可一键切 ③）；本机工具默认须确认 | 出门在外、电脑未开 |
...
以下**不是**当前预览版（0.1.19）能力，写在这里只为讲清方向。
...
已有发送失败后的本轮 `AgentFailover`（③↔④，可落到 ②）。远期补：**首包超时**后静默改用已配置的保底通道（优先已配的 ④/②），顶栏一行短提示即可，不永久改当前 Agent。
